...
const Header = () => {
  ...
  const { isMenuOpen, setIsMenuOpen } = useOnClickOutside(refToMenu);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        dispatch(
			loginUser({ uid, email, displayName, credit: 0 })
        );
		const userDoc = await getDoc(doc(db, "users", auth.currentUser.uid));
		const userData = userDoc.data();
        dispatch(
          loginUser({ uid, email, displayName, credit: userData?.credit || 0 })
        );
        navigate("/browse");
      } else {
        dispatch(logoutUser());
